...
void MFTYPE TargetAddConstantC(MIL_ID SrcImage, MIL_ID DstImage, unsigned long Constant);
...
void MFTYPE TargetAddConstantC(MIL_ID Src, MIL_ID Dst, unsigned long Constant)
   {
   ...
   if ((MbufInquire(Src, M_TYPE, M_NULL)   == FUNCTION_SUPPORTED_IMAGE_DEPTH) &&
      ...
      /* Notify MIL that the buffer were accessed directly using pointers and are now in cache. */
      MbufControl(Src, M_CACHE_CONTROL, M_IN_CACHE); 
      MbufControl(Dst, M_CACHE_CONTROL, M_IN_CACHE); 
