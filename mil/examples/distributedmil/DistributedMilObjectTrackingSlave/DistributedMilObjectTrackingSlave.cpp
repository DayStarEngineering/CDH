 ...
 *        be able to start it in a separate thread on the Host and on the target
 *        system.
 */
...
MIL_INT MFTYPE ModelPositionningHook(MIL_INT HookType, MIL_ID EventId, void MPTYPE *DataExPtr);
MIL_INT MFTYPE GeometricPatternMatchingHook(MIL_INT HookType, MIL_ID EventId, void MPTYPE *DataExPtr);
MIL_INT MFTYPE GrayscalePatternMatchingHook(MIL_INT HookType, MIL_ID EventId, void MPTYPE *DataExPtr);

/******************************************************************************
 *  Slave function: 
 *      - This slave function does grab and processing in a loop and signals
 *        when results are available to the controling computer. 
 *        This prevents the controling computer from having to send each MIL command 
...
MIL_INT MFTYPE ModelPositionningHook(MIL_INT HookType, MIL_ID EventId, void MPTYPE *DataExVoidPtr) 
{
 ...
 */
#if (!M_MIL_LITE)
MIL_INT MFTYPE GeometricPatternMatchingHook(MIL_INT HookType, MIL_ID EventId, void MPTYPE *DataExVoidPtr) 
{
    DataExchangeStruct *DataExPtr = (DataExchangeStruct *)DataExVoidPtr;
    MIL_ID GrabBufferId;
    ...
    MappTimer(M_TIMER_READ+M_SYNCHRONOUS, &DataExPtr->Time);
 ...
 */
#if (!M_MIL_LITE)
MIL_INT MFTYPE GrayscalePatternMatchingHook(MIL_INT HookType, MIL_ID EventId, void MPTYPE *DataExVoidPtr) 
{
    DataExchangeStruct *DataExPtr = (DataExchangeStruct *)DataExVoidPtr;
    MIL_ID GrabBufferId;
    MIL_INT NbFound;
