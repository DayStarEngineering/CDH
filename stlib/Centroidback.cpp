...
int Centroid::run(image* img, centroidList* cntrd)
{
	...
	{
		cntrd->centroid[cnt] = myList.centroid[cnt];
		cnt++;
	}
...
// Apply 0.5 offset and gnomonic projection correction to image pixel locations:
	...
	for (int i = 0; i < myData.xlen; i++)
	{
		for (int j = 0; j < myData.ylen; j++)
		{
			if (img->pixel[i][j] > limit)
			{
				...
				#if PRINT_MODE == 1
				cout<<"ind = "<<ind<<endl;
				cout<<myList.centroid[ind].x<<endl;
				#endif
				// Store the centroid
	...
	while (n > 0)
	{
		...
		for (int i = 1; i < n; i++)
		{
			if (myList.centroid[i-1].intensity < myList.centroid[i].intensity)
			{
				...
				cent = myList.centroid[i-1];
				myList.centroid[i-1] = myList.centroid[i];
				myList.centroid[i] = cent;
				// Set max bound
				newn = i;
			}
		}
